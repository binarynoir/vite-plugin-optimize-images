...
before opening a PR — CI runs the same checks against Node 20, 22, and 24,
and `main` is protected: changes only land through a PR with CI passing.

## Guidelines

- Add or update tests in `test/` for any behavior change.
- Keep the README's Usage/Options sections in sync with any option or
  behavior change.
- Follow the existing naming conventions (see `src/` — full descriptive
  identifiers) rather than introducing new patterns.
